...
function findAvailableClassrooms(timeSlot: TimeSlot, dayOfWeek: DayOfWeek): string[] {
	...
	return classrooms.map((classroom) => classroom.number).filter((number) => !occupiedClassrooms.includes(number));
}
...
function getMostPopularCourseType(): CourseType {
	...
	return Object.entries(typeCounts).reduce((max: [string, number], current: [string, number]) =>
